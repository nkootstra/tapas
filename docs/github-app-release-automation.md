...
| Type | Name | Value |
| --- | --- | --- |
| Variable | `RELEASE_APP_CLIENT_ID` | GitHub App Client ID copied from the App settings page |
...
7. Copy the Client ID into the `RELEASE_APP_CLIENT_ID` repository variable. The Client ID is distinct from the numeric App ID; do not reuse the App ID value.
...
There is exactly one required repository variable, `RELEASE_APP_CLIENT_ID`, and two repository secrets, `RELEASE_APP_PRIVATE_KEY` and `RELEASE_SIGNING_KEY`, for release automation. During migration, the obsolete `RELEASE_APP_ID` variable may remain until one successful release smoke test, then it should be deleted. release-plz creates its release-PR commits through GitHub's API; these GraphQL commits show as Verified. When release-plz returns a non-literal manifest version, the GitHub-signed normalization fallback also uses the GraphQL commit API. The SSH signing key is available only to the isolated direct-tag workflow.
...
If release-plz reports that it did not create or update a pull request while a desired version is pending, preparation fails before editing or enabling auto-merge on any open branch. The workflow binds release-plz's structured pull-request output to the live App-owned pull request instead of adopting an older release branch by discovery alone.
